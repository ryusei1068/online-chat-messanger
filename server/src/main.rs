...
impl Redis_Client {
    ...
    fn is_existing_room<C: ConnectionLike>(
        conn: &mut C,
        room_name: &str,
...
impl Handler {
    fn client_handler(req: Request, peer: SocketAddr) -> Response {
        ...
        */
        ...
        let mut conn = if connection_result.is_ok() {
        ...
        match req.kind {
            ...
            3 => {
                let rooms = Redis_Client::get_rooms(&mut conn, "rooms");
                if rooms.is_ok() {
                    Response {
                        status: "success".to_string(),
                        detail: rooms.unwrap(),
                    ...
                    Response {
                        status: "error".to_string(),
                        detail: "failed to get rooms".to_string(),
                    }
                }
            }
            _ => Response {
